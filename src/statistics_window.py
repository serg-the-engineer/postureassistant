...
class StatisticsWindow(QDialog):
    def __init__(self, stats_service, parent=None):
        ...
        self.setWindowTitle("Posture Statistics")
    ...
    def update_chart(self):
        ...
        if total > 0:
            ...
            self.summary_label.setText(
                ...
                f"• Correct Posture: {summary['CORRECT']/60:.1f} minutes ({correct_pct:.1f}%)\n"
                f"• Incorrect Posture: {summary['INCORRECT']/60:.1f} minutes ({incorrect_pct:.1f}%)"
            )
        else:
            # No data available
            ax = self.figure.add_subplot(111)
            ax.text(0.5, 0.5, 'No posture data available\nfor today', 
